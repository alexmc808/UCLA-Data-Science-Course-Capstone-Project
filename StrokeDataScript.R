...
strokef = strokef[, c(3, 2, 10, 11, 1, 12, 4, 14, 13, 5, 7, 8, 6, 9, 16, 15, 18, 17, 20, 19, 21)]
...
#All census variables, county rank health factors, factor(Hospital), factor(County), and factor(Year).
eq09 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
            as.numeric(`County Rank Health Factors`) + factor(Hospital) + factor(County) + factor(Year), data = readmissions)
summary9 = summary(eq09)
summary9 #Adjusted R^2 = 0.51.

eq10 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
            as.numeric(`County Rank Health Factors`) + factor(Hospital) + factor(County) + factor(Year), data = mortality)
summary10 = summary(eq10)
summary10 #Adjusted R^2 = 0.61 (best model). High median household income, poverty rate --> low RAR. 

#Note: tried adding in other potential predictor variables such as Capacity, and Entity Type, but ultimately did not include them.