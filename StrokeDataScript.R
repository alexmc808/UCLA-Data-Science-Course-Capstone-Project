...
#Import Ischemic Stroke data set (from https://data.world/chhs/06ed38d3-b047-4ae2-aa00-2e43b5491d6e).
...
#Create a new variable for Capacity, Zipcode, and Entity Type data by merging data from health_facility_locations.xlsx. 
...
hospital_facilities_ca = read_excel('CHHSHealthFacilityData.xlsx')
stroke = merge(stroke, hospital_facilities_ca, by.x = "OSHPDID", by.y = "OSHPD_ID", all.x = T)
stroke = stroke[, c(1:11, 34, 37, 57)]
colnames(stroke)[12] = "Capacity (Number of Beds)"
colnames(stroke)[13] = "Zipcode"
colnames(stroke)[14] = "Entity Type"

sum(is.na(stroke$`Capacity (Number of Beds)`)) #There are 140 missing Capacity (Number of Beds), Zipcodes, and Entity Types. Find them manually. 
...
#and Entity Types, so remove them from missing_data and stroke data frames.
...
missing_data = missing_data[, c(1, 12:14)]
...
missing_data = missing_data[, c(1, 5:7)]
names(missing_data) = c("OSHPDID", "Capacity (Number of Beds)", "Zipcode", "Entity Type")

#Merge stroke with missing_data to fill in NAs with missing_data values.
stroke1 = stroke[, -c(12:14)]
stroke2 = stroke[complete.cases(stroke), ] 
stroke2 = stroke2[, c(1, 12:14)]
stroke3 = rbind(stroke2, missing_data)
stroke4 = cbind(stroke1, stroke3, by = 'OSHPDID')
stroke4 = stroke4[, -c(12, 16)]
...
stroke5 = stroke4
stroke5[stroke5$Year == '2013-2014', c(7, 8)] = stroke5[stroke5$Year == '2013-2014', c(8, 7)]
...
stroke11 = stroke11[, -c(15, 16, 17, 19)]
colnames(stroke11)[15] = "County Rank Health Outcomes"
colnames(stroke11)[16] = "County Rank Health Factors"

stroke12 = merge(stroke12, chealth12, by.x = 'County', by.y = '...3', all.x = T)
stroke12 = stroke12[, -c(15, 16, 17, 19)]
colnames(stroke12)[15] = "County Rank Health Outcomes"
colnames(stroke12)[16] = "County Rank Health Factors"

stroke13 = merge(stroke13, chealth13, by.x = 'County', by.y = '...3', all.x = T)
stroke13 = stroke13[, -c(15, 16, 17, 19)]
colnames(stroke13)[15] = "County Rank Health Outcomes"
colnames(stroke13)[16] = "County Rank Health Factors"

stroke14 = merge(stroke14, chealth14, by.x = 'County', by.y = '...3', all.x = T)
stroke14 = stroke14[, -c(15, 16, 17, 19)]
colnames(stroke14)[15] = "County Rank Health Outcomes"
colnames(stroke14)[16] = "County Rank Health Factors"
...
age65a11 = get_acs(geography = "zcta", variables = c(a7584="DP05_0016PE", a6574 = "DP05_0015PE"), 
state = "CA", year = 2011, output = "wide")
age65a11 = age65a11[, c(2, 3, 5)]
age65a11 = age65a11 %>% mutate('Percent Age 65-84' = a6574 + a7584)
age65a11 = age65a11[, c(1, 4)]
age65a11$NAME = gsub("ZCTA5 ", "", age65a11$NAME)

acs11var = merge(mincome11, chci11, by = "NAME")
acs11var = merge(acs11var, pop11, by = "NAME")
acs11var = merge(acs11var, povr11, by = "NAME")
acs11var = merge(acs11var, age65a11, by = "NAME")
...
age65a12 = get_acs(geography = "zcta", variables = c(a7584="DP05_0016PE", a6574 = "DP05_0015PE"), 
state = "CA", year = 2012, output = "wide")
age65a12 = age65a12[, c(2, 3, 5)]
age65a12 = age65a12 %>% mutate('Percent Age 65-84' = a6574 + a7584)
age65a12 = age65a12[, c(1, 4)]
age65a12$NAME = gsub("ZCTA5 ", "", age65a12$NAME)

acs12var = merge(mincome12, chci12, by = "NAME")
acs12var = merge(acs12var, pop12, by = "NAME")
acs12var = merge(acs12var, povr12, by = "NAME")
acs12var = merge(acs12var, age65a12, by = "NAME")
...
age65a13 = get_acs(geography = "zcta", variables = c(a7584="DP05_0016PE", a6574 = "DP05_0015PE"), 
state = "CA", year = 2013, output = "wide")
age65a13 = age65a13[, c(2, 3, 5)]
age65a13 = age65a13 %>% mutate('Percent Age 65-84' = a6574 + a7584)
age65a13 = age65a13[, c(1, 4)]
age65a13$NAME = gsub("ZCTA5 ", "", age65a13$NAME)

acs13var = merge(mincome13, chci13, by = "NAME")
acs13var = merge(acs13var, pop13, by = "NAME")
acs13var = merge(acs13var, povr13, by = "NAME")
acs13var = merge(acs13var, age65a13, by = "NAME")
...
age65a14 = get_acs(geography = "zcta", variables = c(a7584="DP05_0016PE", a6574 = "DP05_0015PE"), 
state = "CA", year = 2014, output = "wide")
age65a14 = age65a14[, c(2, 3, 5)]
age65a14 = age65a14 %>% mutate('Percent Age 65-84' = a6574 + a7584)
age65a14 = age65a14[, c(1, 4)]
age65a14$NAME = gsub("ZCTA5 ", "", age65a14$NAME)

acs14var = merge(mincome14, chci14, by = "NAME")
acs14var = merge(acs14var, pop14, by = "NAME")
acs14var = merge(acs14var, povr14, by = "NAME")
acs14var = merge(acs14var, age65a14, by = "NAME")
...
strokef = strokef[, c(4, 2, 11, 12, 1, 3, 5, 14, 13, 6, 8, 9, 7, 10, 16, 15, 18, 17, 20, 19, 21)]
strokef = strokef[order(strokef$Hospital), ]
...
mutate(percent = prop.table(count) * 100) %>% ggplot(aes(x = factor(`Hospital Ratings`, levels = rev(factor(`Hospital Ratings`))), percent), fill = `Hospital Ratings`) +
geom_col(fill = c('purple', 'pink', 'light blue')) + geom_text(aes(label = sprintf("%.1f%%", percent)), hjust = 0.2, vjust = 1, size = 5) + theme_bw() +  
...
p5 = boxplot(readmissions$`Risk Adjusted Rate` ~ readmissions$Year, names = c('2011-2012', '2012-2013', '2013-2014', '2014-2015'), 
xlab = "Year", ylab = "Risk Adjusted Rate", main = "Boxplot of Risk Adjusted Rates by Year (Readmissions)", 
cex.main = 1, par(cex.axis = 0.9))
p6 = boxplot(mortality$`Risk Adjusted Rate` ~ mortality$Year, names = c('2011-2012', '2012-2013', '2013-2014', '2014-2015'), 
xlab = "Year", ylab = "Risk Adjusted Rate", main = "Boxplot of Risk Adjusted Rates by Year (Mortality)", cex.main = 1, par(cex.axis = 0.9))
...
#All census variables and county rank health factors.
eq01 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
            as.numeric(`County Rank Health Factors`), data = readmissions)
summary1 = summary(eq01)
summary1 #Adjusted R^2 = 0.05. High median household income, County Rank Health factors --> high RAR. High CHCI --> low RAR.

eq02 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`), data = mortality)
summary2 = summary(eq02)
summary2 #Adjusted R^2 = 0.01. High County Rank Health factors --> high RAR. High median household income --> low RAR.

#All census variables, county rank health factors, and factor(County).
eq03 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`) + factor(County), data = readmissions)
summary3 = summary(eq03)
summary3 #Adjusted R^2 = 0.17. High County Rank Health factors --> high RAR. Del Norte, Fresno, Kern, Kings, Lake, Madera, Mendocino, Merced, Monterey
#Sacramento, San Joaquin, Santa Barbara, Shasta, Siskiyou, Solano, Sonoma, Stanislaus, Tehama, Yolo, Yuba --> low RAR. 

eq04 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`) + factor(County), data = mortality)
summary4 = summary(eq04)
summary4 #Adjusted R^2 = 0.22. Humboldt, Madera, San Benito, SLO, Siskiyou, Yolo --> high RAR. High median household income, poverty rate, Butte, Del Norte, Imperial, LA, Monterey,
#Solano --> low RAR.

#All census variables, county rank health factors, and factor(Hospital).
eq05 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`) + factor(Hospital), data = readmissions)
summary5 = summary(eq05)
summary5 #Adjusted R^2 = 0.50. High County Health Rank Factors --> high RAR. 

eq06 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`) + factor(Hospital), data = mortality)
summary6 = summary(eq06)
summary6 #Adjusted R^2 = 0.61. #High County Health Rank Factors --> high RAR. High median house hold income, poverty rate --> low RAR.

#All census variables, county rank health factors, factor(Hospital), and factor(County).
eq07 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
as.numeric(`County Rank Health Factors`) + factor(Hospital) + factor(County), data = readmissions)
summary7 = summary(eq07)
summary7 #Adjusted R^2 = 0.50. High County Health Rank Factors --> high RAR.
stargazer(eq07, type = "text")

#Find hospitals most significantly positively and negatively associated with risk adjusted rate.
coef_table7 = summary7$coefficients
top_np7 <- coef_table7[order(coef_table7[, 4], decreasing = F)[1:60], ]
top_np7 #Greater El Monte Hospital most significantly positively associated with RAR; Frank R. Howard Memorial Hospital most significantly
#negatively associated with RAR.

eq08 = lm(`Risk Adjusted Rate` ~ `Median Household Income Estimate` + CHCI + `Population Estimate` + `Poverty Rate` + `Percent Age 65-84` + 
            as.numeric(`County Rank Health Factors`) + factor(Hospital) + factor(County), data = mortality)
summary8 = summary(eq08)
summary8 #Adjusted R^2 = 0.61. High County Health Rank Factors --> high RAR. High median household income, poverty rate --> low RAR.
stargazer(summary8$coefficients, type = "text")

#Find hospitals most significantly positively and negatively associated with risk adjusted rate.
coef_table8 = summary8$coefficients
top_np8 = coef_table8[order(coef_table8[, 4], decreasing = F)[1:5], ]
top_np8

#Note: tried adding in other potential predictor variables such as Capacity, Entity Type, and factor(Year) but ultimately did not include them.